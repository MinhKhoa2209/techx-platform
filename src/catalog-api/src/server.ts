...
export function createCatalogServer(options: CatalogServerOptions = {}): Server {
  ...
  const server = createServer({
    ...
    if (request.method !== 'GET') {
      sendError(response, 405, 'METHOD_NOT_ALLOWED', 'Only GET is supported for this resource.', requestId);
      return;
    }

    if (path === '/healthz') {
      sendJson(response, 200, { status: 'ok', service: 'catalog-api' }, requestId);
      return;
    }

    if (path === '/readyz') {
      if (ready) {
        sendJson(response, 200, { status: 'ready', service: 'catalog-api' }, requestId);
      } else {
        sendError(response, 503, 'NOT_READY', 'Catalog API is shutting down.', requestId);
      }
      return;
    }
    ...
    if (productMatch) {
      ...
      if (!product) {
        sendError(response, 404, 'PRODUCT_NOT_FOUND', 'Product was not found.', requestId);
        return;
      }
    ...
    sendError(response, 404, 'ROUTE_NOT_FOUND', 'Route was not found.', requestId);
  });
